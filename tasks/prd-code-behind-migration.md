...
- All E2E tests continue to pass after migration
...
**Acceptance Criteria:**
- [ ] Create `PhonicsCard.razor.cs` as a `partial class PhonicsCard`
...
- [ ] Build passes
- [ ] All E2E tests pass (`dotnet test tests/NowThenNext.Tests.E2E`)
...
**Acceptance Criteria:**
- [ ] Create `StorageWarningBanner.razor.cs` as a `partial class StorageWarningBanner`
...
**Acceptance Criteria:**
- [ ] Create `PhonicsSoundList.razor.cs` as a `partial class PhonicsSoundList`
- [ ] Move all fields, parameters, methods, enums, and lifecycle overrides from the `@code` block into the code-behind file
...
**Acceptance Criteria:**
- [ ] Create `PhonicsPhases.razor.cs` as a `partial class PhonicsPhases`
...
**Acceptance Criteria:**
- [ ] Create `ImageTile.razor.cs` as a `partial class ImageTile`
...
- FR-1: Each migrated `.razor` file must have a corresponding `.razor.cs` file using `partial class` (no explicit `ComponentBase` inheritance needed — Blazor infers it from the `.razor` file)
...
- FR-7: Fields and methods referenced in `.razor` markup can remain `private` in the code-behind since both files are part of the same `partial class`
...
- Fields/properties referenced in `.razor` markup can remain `private` in the code-behind (the generated Razor code is part of the same partial class)
